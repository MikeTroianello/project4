...
router.post("/createstory", isLoggedIn, (req, res, next) => {
  ...
  Story.create()
    .then(storyDoc => {
      const newStory = new Story({ title, content, creatorId, creatorName, idOfLastPage, teaser, pageNumber })
...
//SEE OTHERS' PROFILES
router.get('/view-profile/:creatorName', isLoggedIn, (req,res,next)=>{
  const id = req.params.creatorId
  const name = req.params.creatorName
  console.log("id = ", id)
  console.log(req.params.creatorId)
  console.log("name = ", name)
  console.log(req.params.creatorName)
  User.findById(name).then(userPulled=>{
